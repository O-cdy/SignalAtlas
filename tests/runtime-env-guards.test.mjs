...
describe('variant env guards', () => {
  ...
  it('reuses buildVariant for SSR, Tauri, and localhost fallback paths', () => {
    ...
    assert.equal(buildVariantUses.length, 3, `Expected three direct buildVariant fallbacks, got ${buildVariantUses.length}`);
    ...
    assert.match(
      ...
      'non-variant production hosts such as *.vercel.app should honor the build-time variant',
    );
  });
});
